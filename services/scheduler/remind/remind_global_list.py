...
async def remind_empty_tag_for_global_list_every_week():
    ...
    # Fetch Global List
    lists = await get_all_empty_tag_for_global_list()
    if lists:
        user_before = None
        global_list = ""

        for idx, dt in enumerate(lists):
        ...
        if admins:
            for admin in admins:
                await send_tele_chat(
                    ...
                    msg=f"[ADMIN] Hello {admin.username}, I just checked the global list and found a total of {total_list} list still doesnt have a tag, from {total_user} users."
                )

async def remind_empty_global_list_every_week():
    # Fetch Admin Contact
    admins = await get_all_admin_contact()
    total_user = 0
    total_list = 0
    ...
    # Fetch Global List
    lists = await get_all_empty_global_list()
    if lists:
        user_before = None
        global_list = ""
            ...
            if user_before != dt.username:
                ...
                if user_before is not None:
                    ...
                    if dt.telegram_is_valid == 1:
                        await send_tele_chat(
                            tele_id=prev_tele_id,
                            msg=f"Hello {user_before}, I found that some of your global list doesnt have at least one pin / marker attached:\n\n{global_list}"
                        )
        ...
        if admins:
            for admin in admins:
                await send_tele_chat(
                    tele_id=admin.telegram_user_id,
                    msg=f"[ADMIN] Hello {admin.username}, I just checked the global list and found a total of {total_list} list still doesnt have at least one pin / marker attached, from {total_user} users."
                )