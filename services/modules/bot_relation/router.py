...
from services.modules.bot_relation.command import post_check_bot_relation, post_create_bot_relation, sign_out_bot_relation
...
@router_bot_relation.post("/api/v1/bot_relation/sign_out", response_model=dict, 
    summary="Delete Check Bot Relation (MySql)",
    description="This request is used to delete bot relation (sign out)",
    tags=["Bot Relation"],
    status_code=200,
    responses={
        200: {
            "description": "Successful delete the bot relation",
            "content": {
                "application/json": {
                    "example": {
...
async def sign_out_bot_relation_api(request : Request):
    try:
        data = await request.json()
        return await sign_out_bot_relation(data=data)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
